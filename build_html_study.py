...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .fc-status-badge.confirm {{ color: #0d9488; }}
    .fc-actions {{
      display: flex;
      justify-content: center;
      gap: 0.75rem;
    ...
    .fc-weak-item:hover {{ border-color: #ea580c; }}
    ...
    .exam-option.correct {{ border-color: var(--success); background: #ecfdf5; }}
    .exam-option.wrong {{ border-color: #ef4444; background: #fef2f2; }}
    .exam-option.missed {{ border-color: var(--success); background: #ecfdf5; opacity: 0.85; }}
    .exam-nav-btns {{
      display: flex;
      flex-wrap: wrap;
      gap: 0.75rem;
...
<body>
  ...
  <div class="layout">
    <aside class="sidebar" id="sidebar">
      ...
      <div id="exam-nav" style="display:none">
        ...
        <button type="button" class="filter-btn" id="btn-clear-exam-history" style="margin-top:0.5rem;color:#b91c1c">Wis historiek</button>
      </div>
  ...
  <script>
    ...
    function loadFcProgress() {{
      try {{
        const raw = localStorage.getItem(STORAGE_FC_PROGRESS);
        if (raw) {{
      ...
      const migrated = {{}};
      legacy.forEach(id => {{
        migrated[id] = {{ state: 'mastered', fails: 0 }};
      }});
      return migrated;
    }}
    ...
    function updateStats() {{
      ...
      document.getElementById('fc-stats').textContent =
        mastered + ' / ' + pool.length + ' beheerst in deze selectie';
      document.getElementById('fc-progress-fill').style.width = (mastered / total * 100) + '%';
      document.getElementById('fc-progress-fill').classList.toggle('confirm', confirm > 0 && mastered < pool.length);
      const statsEl = document.getElementById('fc-session-stats');
      statsEl.innerHTML =
    ...
    function showExamResults(entry) {{
      ...
      document.getElementById('exam-review-list').innerHTML = entry.answers.map((a, i) => {{
        ...
        return `<div class="exam-review-item ${{cls}}">
          <strong>Vraag ${{i + 1}}:</strong> ${{escapeHtml(a.q)}}<br>
          Jouw antwoord: ${{chosenText}}<br>
          ${{a.chosen !== a.correct ? '<span style="color:#b91c1c">Correct: ' + correctText + '</span>' : '<span style="color:var(--success)">Correct</span>'}}
        </div>`;
      }}).join('');
